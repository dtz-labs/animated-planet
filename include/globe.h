 ...
 * (horizontal). Each point's screen-y is constant under Y-rotation. X offsets
 * are precomputed per distinct projected radius, so per-frame projection is a
 * front test plus a table lookup. All dots are identical (white).
 */
#ifndef GLOBE_H
#define GLOBE_H
...
/* Combined hot-path projection: returns 1 and writes x/y if the point is on the
 * front hemisphere, returns 0 otherwise. */
u8 globe_project_front(u8 i, u8 theta, u8 *x, u8 *y);

/* Hotter variant for renderers that already cached globe_y(i). */
